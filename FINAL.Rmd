...
filter(selling_price<700000,
       km_driven<700000) %>%
  ggplot(aes(x=fuel,y=selling_price,fill=fuel))+geom_boxplot()+ stat_summary(fun="mean")
...
vehicles %>%
  ...
  ggplot(aes(x=selling_price, y=km_driven)) + geom_point() +geom_smooth()+ labs(title="Selling price vs Distance Driven", x="Selling price", y="distance driven")
...
```{r}
augment(vehicles_fit, new_data = vehicles_test) %>%
...
```{r}
augment(vehicles_fit_svm, new_data = vehicles_test) %>%
  ggplot(aes(selling_price, .pred)) +
  geom_abline() +
  geom_point(alpha = 0.5)
```
...
augment(vehicles_fit_bt, new_data = vehicles_test) %>%
  ggplot(aes(selling_price, .pred)) +
  geom_abline() +
  geom_point(alpha = 0.5)
